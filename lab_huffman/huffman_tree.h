	...
	private:
		...
		class TreeNode {
			public:
				 ...
				 *	with.
				 */
				TreeNode( Frequency theFreq ) : freq( theFreq ), 
				 ...
				 */
				TreeNode( int frequency ) : freq( frequency ), 
					left( NULL ), right( NULL ) {
					// nothing
				}
		};
		...
		std::map<char, std::vector<bool>> bitsMap; /**< Standard map that maps characters to their encoded values. */
};
#endif
