...
void testShortestPath(Graph* graph, Vertex start, Vertex end, string fileName)
{
    ...
    cout << "Shortest path from " << start << " to " << end << ": "
         << pathSize << (pathSize == 1 ? " edge." : " edges.") << endl;
}
...
int main(int argc, char* argv[])
{
    ...
    {
        Vertex start = PremadeGraphs::getStartVertex(type);
        Vertex end = PremadeGraphs::getEndVertex(type);
        testShortestPath(graph, start, end, type);
    }
    else if(test == "path" && type == "random")
        testShortestPath(graph, 0, numVertices - 1, type);
