...
class heap {
    public:
         ...
         *
         * By default, the higherPriority() function behaves like
         * operator<, so if T = int and a = 3 and b = 5, higherPriority(a,
         * b) is true (a compares smaller than b, thus it has higher
         * priority) and higherPriority(b, a) is false (b does not compare
         ...
         */
        template <class Type, class Comp>
        friend std::ostream & operator<<( std::ostream & out, const heap<Type, Comp> & toPrint );
